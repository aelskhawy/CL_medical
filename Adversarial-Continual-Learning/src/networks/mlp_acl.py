...
class Private(torch.nn.Module):
    def __init__(self, args):
        ...
        ## P modules, one for each task, to generate task specific features
...
class Net(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        ## task specific heads (small p modules) that takes both Zp and Zs and produce the final task output
        ...
        for i in range(self.num_tasks):
            self.head.append(
                torch.nn.Sequential(
                    ...
                    torch.nn.ReLU(inplace=True),
                    torch.nn.Linear(self.hidden2, self.taskcla[i][1])  # 2 units for 2 classes
                ))
    ...
    def forward(self,x_s, x_p, tt, task_id):
        ...
        # concat zp and zs and then for each single input in the patch, pass it through the respective task head to get
        # the final output
        # tt[i] = task_num (to access the respective head) for the ith input in the batch
